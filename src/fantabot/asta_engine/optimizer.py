...
The builder is greedy — it adds the player with the best marginal gain per credit, honours
...
from .legality import SchemaLegality, fieldable_schemi
...
def _build(
    ...
    while len(picked) < rules.size:
        ...
        best = max(
            candidates,
            key=lambda p: _marginal_gain(p.id, picked, value, teams, lam, rho) / _cost(p.id, prices),
        )
