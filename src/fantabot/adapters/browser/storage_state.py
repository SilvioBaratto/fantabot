...
What is left is one path, read by ``application/auth_login`` alone. It stays a plain
function over ``settings`` and imports nothing from ``fantabot.adapters.persistence``
on purpose: the login flow sits on this import chain and ``interface/app.py`` sits on
that one, so ``fantabot --help`` has to work before a database exists. The login flow
does reach the database — deliberately — but only inside its command body, so it can
*report* an unreachable one rather than fail to import.
...
constraint is unchanged, only the route to it. The two modules were called
``login.py`` and ``cli.py`` then, which is how this paragraph named them until
2026-09-24.)
"""
