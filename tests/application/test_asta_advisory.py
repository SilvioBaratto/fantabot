...
**The replay path stays in the CLI, and that is why `events` is a parameter.** The parity
phase's spec (archived, not in this checkout) T20 calls `--replay` developer machinery;
making the fold take events rather than read them keeps one fold over two sources instead
of two folds. What the app supplies is a live ledger, what
