...
`statistiche.DEFAULT_SEASONS` stop at 2025/26 while the season being played is 2026/27,
so a run that omits `--season` scrapes last season and reports success. `scrapables`
reads each scraper's **own** list rather than keeping a copy, which is why a test here
can flip the staleness by moving the scraper's list and not this module's.
...
def test_the_resolved_default_differs_by_table() -> None:
    """The trap, stated as a comparison: one of the three already carries 2026/27."""
    assert clean_scrape("quotazioni", []).seasons != clean_scrape("voti", []).seasons


# -- the stale default, carried where a screen can read it -----------------------------


def test_voti_and_statistiche_default_to_a_season_that_is_over() -> None:
    assert table("voti").default_is_stale is True
    assert table("statistiche").default_is_stale is True


def test_quotazioni_does_not() -> None:
...
def test_staleness_is_read_from_the_scraper_and_never_copied(
    ...
    """Fix the scraper's own list and the warning goes away with no edit here.

    A copy in this module would be a second default to keep in step, which is the
    defect rather than the report of it.
    """
    from fantabot.adapters.scraping import voti

    monkeypatch.setattr(voti, "DEFAULT_SEASONS", [*voti.DEFAULT_SEASONS, "2026/27"])
    assert table("voti").default_is_stale is False
    assert table("voti").default_seasons[-1] == "2026/27"
